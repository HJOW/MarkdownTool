...
6. **인쇄**를 누르면 A4 페이지 서식을 적용한 별도 문서를 준비한 뒤 브라우저의 인쇄 창을 엽니다. 브라우저가 팝업을 차단하면 이 사이트의 팝업을 허용한 뒤 다시 시도합니다.
7. 하단에는 현재 원문의 줄 수, UTF-8 바이트 수, 캐릭터셋이 표시됩니다. 언어 선택에서 **English / 한국어**를 고르고, **다크 모드 / 밝기 모드** 버튼으로 테마를 전환합니다.
...
화면 언어는 영어를 기본값으로 사용하고, 브라우저의 시스템 언어가 한국어이면 한국어로 시작합니다. 사용자가 하단에서 고른 언어는 해당 브라우저에 기억합니다. 테마는 처음에는 브라우저의 밝기 설정을 따르고, 사용자가 바꾸면 해당 브라우저에 기억합니다. 브라우저 저장소가 차단된 경우에도 언어와 테마 전환은 사용할 수 있습니다.

## WebMCP 지원

[WebMCP 초안](https://github.com/webmachinelearning/webmcp)의 `document.modelContext.registerTool()`을 지원하는 브라우저에서는 AI 에이전트가 현재 페이지의 편집·보기 기능을 도구로 호출할 수 있습니다. 파일 저장과 파일 불러오기는 의도적으로 도구에 포함하지 않습니다. WebMCP를 지원하지 않거나 도구 등록 권한이 없는 브라우저에서는 등록을 건너뛰며, 기존 화면 기능은 그대로 사용할 수 있습니다.

| 도구 | 기능 | 입력 |
| --- | --- | --- |
| `get_document_state` | Markdown 원문, 수정 여부, 통계와 현재 화면 상태 조회 | 없음 |
| `set_markdown_source` | 파일을 열지 않고 현재 편집 원문 교체 | `source` 문자열 |
| `create_new_document` | 화면과 같은 변경 확인을 거쳐 빈 문서 만들기 | 없음 |
| `set_view_mode` | 편집, 보기, 같이 보기 모드 전환 | `mode`: `edit`, `view`, `split` |
| `set_theme` | 밝기 또는 다크 테마 적용 | `theme`: `light`, `dark` |
| `set_language` | 영어 또는 한국어 화면 적용 | `language`: `en`, `ko` |
| `print_document` | 인쇄용 문서를 만들고 브라우저 인쇄 창 열기 | 없음 |

WebMCP로 원문을 교체해도 현재 파일명, BOM과 줄바꿈 규칙은 유지됩니다. 새 문서 만들기는 저장하지 않은 변경이 있으면 화면 버튼과 같은 확인 창을 거칩니다. 인쇄 도구의 팝업 허용 여부와 최종 인쇄 동작은 브라우저 정책을 따릅니다.
...
- 인쇄는 밝기 테마의 독립 HTML 문서를 사용합니다. A4 세로 용지의 각 페이지에 10mm 여백을 두고, 제목 뒤에서 페이지가 나뉘거나 표·코드·수식·Mermaid 같은 블록이 중간에서 갈라지는 일을 가능한 범위에서 피합니다. 한 페이지보다 큰 블록은 브라우저가 나누며, 실제 용지와 머리글·바닥글 등의 최종 설정은 브라우저 인쇄 창에서 정합니다.
...
| `src/js/localization.js` | 영어·한국어 화면 문구, 시스템 언어 감지와 번역 처리 |
...
검사는 빌드 후 포트 9804에서 임시 서버를 실행합니다. 빈 문서와 UTF-8 통계, 줄번호와 Markdown 문법 강조, 모드별 편집 제한, 영어·한국어 선택과 시스템 언어 감지, WebMCP 도구 등록과 실행, 고급 서식, 위험한 HTML 제거, 파일 입출력, BOM·줄바꿈 보존, 저장 취소, 오프라인 HTML 열기, PDF 쪽 나누기, 인쇄용 페이지 서식, 테마, 좁은 화면 및 정적 파일 제공 범위를 확인합니다. 다운로드 창과 인쇄 창 자체의 모양, 저장 위치 및 출력 장치는 브라우저 및 운영체제 설정을 따릅니다.
