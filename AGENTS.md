...
- 처음 열면 파일명 `notitle`인 빈 Markdown 문서가 편집 모드로 시작한다. 편집 모드와 같이 보기 모드에서 원문을 바꿀 수 있다. 보기 모드에서만 편집기를 읽기 전용으로 바꾸어 편집을 막는다. 같이 보기에서 원문을 고치면 입력이 잠시 멈춘 뒤 오른쪽 미리보기를 다시 만든다.
